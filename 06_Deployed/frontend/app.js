(function () {
  ...
  function updateScenarioSummary() {
    ...
    document.getElementById("scenarioSummary").innerHTML =
      ...
      "Final Test Evaluation View &middot; " + horizonLabel;
  ...
  elNewRun.addEventListener("click", async function () {
    ...
    elBatchLine.textContent = "Đang tạo run + load dữ liệu thật (Final Test Evaluation View)...";
    try {
      ...
      elBatchLine.textContent = "Run " + res.run_id + " sẵn sàng (" + res.total_requests + " request thật từ Final Test Evaluation View)." + actualNote;
      ...
      document.getElementById("crSlice").textContent = res.total_requests.toLocaleString("en-US") +
        ...
        (availableCount || 195506).toLocaleString("en-US") + " tổng";
      if (res.constants) {
  ...
  async function loadCompare() {
    ...
    try {
      ...
      document.getElementById("compareSource").textContent = "Nguồn: " + r.source + " (5 seed, 195,506 request/seed, Final Test)";
  ...
  async function loadProvenance() {
    try {
      ...
      var testEntry = r.runtime_dataset;
      var testSha = testEntry && testEntry.sha256 ? testEntry.sha256.slice(0, 16) + "..." : "Không có sẵn";
      var engineSha = (r.engine_source && r.engine_source.files["policies.py"] &&
        r.engine_source.files["policies.py"].sha256 || "Không có sẵn");
      document.getElementById("provenanceStrip").innerHTML =
        ...
        "<span>Final Test Evaluation View SHA-256: <b>" + testSha + "</b></span>" +
        "<span>Số dòng: <b>" + (testEntry && testEntry.rows || "Không có sẵn") + "</b></span>";
    } catch (e) {
